...
- **End-to-End Features**: Use `.claude/commands/feature-builder.md` (Adapt patterns for TanStack)
...
- `src/lib/`: Shared utility functions and clients (QueryClient, etc.).
...
3. **Routing**: Add new routes in `src/routes/`. TanStack Router will generate types automatically.
4. **API Hooks**: Use TanStack Query. Standardize on `use-feature-name.ts` naming convention.
5. **Styling**: Use Tailwind 4. Use `.glass`, `.grid-bg`, and `.ring-soft` for consistent effects.
